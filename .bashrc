...
if ${use_color} ; then
	if [[ ${EUID} == 0 ]] ; then
		PS1+="\[\033[091m\]\h\[\033[094m\] \w\[\033[093m\]\$(git branch 2> /dev/null | sed -e '/^[^*]/d' -e 's/* \(.*\)/(\1)/')\[\033[094m\] \\$\[\033[00m\] "
	else
		PS1+="\[\033[092m\]\u@\h\[\033[094m\] \w\[\033[093m\]\$(git branch 2> /dev/null | sed -e '/^[^*]/d' -e 's/* \(.*\)/(\1)/')\[\033[094m\] \\$\[\033[00m\] "
	fi
